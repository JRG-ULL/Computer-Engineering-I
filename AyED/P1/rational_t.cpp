...
bool rational_t::is_int() const { return get_num() % get_den() == 0; }
...
rational_t rational_t::cube() {
  return rational_t(get_num() * get_num() * get_num(),
                    get_den() * get_den() * get_den());
}

// Operador Inverso
rational_t rational_t::inverse() { return rational_t(get_den(), get_num()); }

// Operador simplificación
rational_t rational_t::simplify() {
  return rational_t(get_num() / __gcd(get_num(), get_den()),
                    get_den() / __gcd(get_num(), get_den()));
}
