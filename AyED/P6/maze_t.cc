...
bool maze_t::solve_(const int i, const int j) {
  ...
  if (matrix_(i, j) == matrix_(i_end_, j_end_)) return true;
  ...
  for (int position = N; position <= W; position++) {
    if (is_ok_(i + i_d[position], j + j_d[position])) {
      if (solve_(i + i_d[position], j + j_d[position])) {
        matrix_(i, j) = PATH_ID;
        return true;
      }
    }
