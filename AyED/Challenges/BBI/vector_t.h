...
class vector_t {
 public:
  ...
  int BBI(const int, int, int) const;
...
template <class T>
int vector_t<T>::BBI(const int val, int from, int to) const {
  // Base case
  if (from > to) {
    return -1;
  }
  // Get middle index
  int mid_inx = (from + to) / 2;
  // 'val' is the middle value
  if (at(mid_inx) == val) {
    return mid_inx;
  }
  // 'val' is lower than the middle value
  else if (val < at(mid_inx)) {    
    return BBI(val, from, mid_inx - 1);
  } 
  // 'val' is higher than the middle value
  else {    
    return BBI(val, mid_inx + 1, to);
  }
}
